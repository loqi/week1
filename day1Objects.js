...
//      if no such reference exists, and finally return that value you just wrote as the resolution
...
//            mythicalAnimal['newPropertyToAdd'] = true;
//      which is not what we want. That code would attach the property `'newPropertyToAdd':true`
//      to our mythical creature, instead of the property `'can fly':true`. Even if we'd put the
//      property name straight into our code, it wouldn't work:
...
//      as if it had been `mythicalAnimal.can; fly = true;`, which is also not what we want.
//      All current versions will see `fly` and give up because the expression `mythicalAnimal.can`
//      is not nested within a bigger expression, so once it's resolved, the next piece of code is
//      expected to be whatever can sensibly come after that expression. Since this expression is not
//      nested within some bigger statement, the expression all by itself is the statement. In this
//      case, nothing but a semicolon or a newline can signal the end of the statement and the
//      beginning of a new statement, so when it sees an identifier, it gives up on our code.
...
//      data heap with the character sequence described by the stuff between the matched pair of
//      quotation marks. After that, resolve the expression represented by that bit of string
//      literal code to be a reference to that string you just made. M'kay, love?"
...
//    just gets used as-is. So the string 'collects' gets searched within the object referenced by the
//    variable mythicalAnimal, no such property name is found, so it gets created when the '='
...
// The earlier version creates a string 'can fly' and stuffs it into the variable `newPropertyToAdd`
// with a reference to that brand new string. In JavaScript, a string counts as a primitive data
// type, meaning it is (or the interpreter puts on a convincing show that it is) stored straight
// inside a variable. Later, we'll go deeper into this, but for now, just think of strings as
// behaving like numbers or booleans. Even gigantic strings live happily inside a single variable
// data value, while even tiny objects only have their memory reference fitting inside a variable as
// its data value. Perhaps there is black magic here afterall! Now back to the example.
// The next line of the earlier version resolves the variable name `newPropertyToAdd` to whatever is
// stored in that variable: the string `'can fly'`. Likewise in the second version, the string
// literal `'collects'` calls for the creation of a new string and resolves to the value of that
// string, all in one step. This string then serves as a property name within the object describing
// our dragon. 
//
//
//
...
//
// The dot notation asks the interpreter to convert the identifier characters after the dot into a
// string, in this case the string 'thisIsAVariable' in our mythicalAnimal object. We don't have
// such a property, so the expression `mythicalAnimal.thisIsAVariable` resolves to the value
// `undefined`, which is then passed to the console.log method as its argument. console.log() then
// logs what it got (the undefined value), just as requested, which means to show the word "udefined".
...
//        obj['key'] = value;
//    Anything that can possibly come after the dot is valid inside a string literal, but not anything
//    that could possibly go inside the quotes is valid after a dot. And certainly not everything that
//    could go inside the square brackets can go after a dot. Think of dot notation as the pretty way
//    to avoid bracket notation as often as you can possibly get away with it, but no more often than
//    that! Think of bracket notation as the ugly cousin that gets shite done no matter what, but
//    has slightly bad breath.


// OBJECT LITERALS
// One last exercise! So far we've been going through and creating different properties one by one.
// Let's use object literal notation to create a whole object at once! Here's an example.
//        var goalsForTheYear = {
//          cycling: '5000 miles',
//          climbing: '100 days',
//          giving: 'selflessly'
//        };
// These lines of code mean create a new object somewhere in memory. Create six strings somewhere in
// memory. Populate that object with three properties made of those six strings, three as key, three
// as value. Create a new variable. And finally load that variable with the reference to that newly
// created object. At the end of all this, there will be six strings, one object, and one variable
// managed by the JavaScript interpreter, all accessible to later lines of code through the reference
// in the goalsForTheYear variable. You could have quotation marks around 'cycling', 'climbing' and
// 'giving', for an exactly identical effect. You'll need those quotes around any property name in an
// object literal that doesn't conform to JS identifier rules, such as a property name with multiple
// words or weird characters.
//
//    18. Create a new variable, mythicalAnimal2, and set it equal to an object literal with these
//        keys, and any values of your choosing:
//              type
//              name
//              likes
...
//          `'best friend'` if you roll that way). This property's value should be a reference to
//          one of the existing mythical creatures. If this creature is feeling lonely or
//          narcissistic, you can make it point to itself. Don't worry, you won't break math or
//          physics by doing this; you'll merely be copying an object reference into the value field
//          of a property of an object. Boom. Done. If you try to go all the way to the end of that
//          chain of friend of friend of friend, that's when you break physics, or have to break out
//          of an infinite loop, whichever comes first.
//      20. Do #19 again, but this time specify the best friend by nesing an inner object literal for
//          a new creature complete with all attributes. Such a creature will persist in memory, but
//          can only be touched by code like `socialMythicalAnimal.imaginaryFriend.superpower`. You
//          can later create a shortcut like `var shyCreature = socialMythicalAnimal.imaginaryFriend;`
//          which will copy that inner creature's reference address to a variable so you can have a
//          more direct path to it whenever you need it. like so, 'shyCreature.superpower`.
